    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                var vhcleImg = result[i].vhcleImg;
                result[i].vhcleImgUrl = vhcleImg; // 새로운 필드에 이미지 URL 추가
            }
			...
			//render 함수는 ag-grid 내부적으로 각 셀에 데이터와 함께 호출
			//행, 열 정보를 파라미터로 자동으로 전달받음
			//함수내부에서 params 객체를 통해 접근
            gridOptions.columnDefs.splice(indexToInsert, 0 ,imgColumn);
            // gridOptions.columnDefs.push({ field: "vhcleImgUrl", headerName: "이미지", cellRenderer: renderer });
